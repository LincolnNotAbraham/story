...
def build_complete_story_tree(db: Session, story: Story) -> CompleteStoryResponse:
    ...
    # Converter nós para resposta
    all_nodes = {}
    for node in nodes:
        all_nodes[node.id] = CompleteStoryNodeResponse(
            id=node.id,
            content=node.content,
            is_ending=node.is_ending,
            is_winning=node.is_winning or False,
    ...
    return CompleteStoryResponse(
        id=story.id,
        title=story.title or f"História sobre {story.theme}",
        theme=story.theme or "fantasia",
        created_at=story.created_at,
        root_node=all_nodes[root_node.id],
        all_nodes=all_nodes
    )
